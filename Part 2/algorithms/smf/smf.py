...
class SessionMF:
    '''
    SessionMF( factors=100, batch=32, learn='adagrad', learning_rate=0.01, momentum=0.0, regularization=0.5, dropout=0.0, skip=0, samples=2048, activation='linear', objective='bpr_max', epochs=10, shuffle=-1, last_n_days=None, session_key = 'playlist_id', item_key= 'track_id', user_key= 'playlist_id', time_key= 'pos' )

    Parameters
    -----------
    '''
    
    def __init__( self, factors=100, batch=32, learn='adagrad', learning_rate=0.01, momentum=0.0, regularization=0.5, dropout=0.0, skip=0, samples=2048, activation='linear', objective='bpr_max', epochs=10, shuffle=-1, last_n_days=None, session_key = 'playlist_id', item_key= 'track_id', user_key= 'playlist_id', time_key= 'pos' ):
    ...
    def train(self, trainmap, test=None):
        '''
        ...
        for j in range( self.epochs ):
            ...
            svec = sparse.lil_matrix( (self.batch, self.num_items) , dtype=self.floatX )
            #smat = np.zeros( (self.batch, self.num_items) ).astype( self.floatX )
            #smat = sparse.lil_matrix( (self.batch, self.num_items) , dtype=self.floatX )
            ...
            while not finished:
                ...
                if self.samples > 0:
                    ...
                    ex = np.hstack( [targets, additional] )
                else:
                    ex = targets
                
                stmp = time.time()
                #loss += self.train_model_batch( svec, items, users, ex ) )
                loss += self.train_model_batch( svec, items, ex )
...
#                     itemss = set()
        ...
        print( 'avg_time_fact: ',( (avg_time / avg_count ) if avg_count > 0 else 0 ) )
    ...
    def _generate_predict_function(self):
        ...
        inp = [s, i] #+ [u]
        self.predictf = theano.function(inputs=inp, outputs=pred )
    ...
    def _generate_predict_batch_function(self):
        ...
        inp = [s, i] #+ [u]
        self.predict_batch = theano.function(inputs=inp, outputs=pred ) #, updates=updates )
    ...
    def adagrad(self, updates, loss, param_list, learning_rate=1.0, epsilon=1e-6, momentum=0.0):
        ...
        for param, grad in zip(param_list, all_grads):
            ...
            if momentum > 0: 
                ...
                velocity2 = momentum * velocity - delta
                updates[velocity] = velocity2
                updates[param] = param + velocity2
            else:
                updates[param] = param - delta
                
    def adagrad_sub(self, updates, loss, param_list, subparam_list, idx, learning_rate=1.0, epsilon=1e-6, momentum=0.0 ):
        ...
        for i in range(len(all_grads)):
            ...
            updates[accu] = T.set_subtensor(accu_s, accu_new) 
            ...
            if momentum > 0:
                ...
                updates[param] = T.inc_subtensor(subparam, velocity2 )
            else:
                updates[param] = T.inc_subtensor(subparam, - delta )
    ...
    def predict( self, name=None, tracks=None, playlist_id=None, artists=None ):
        ...
        svec = np.zeros(self.num_items, dtype=np.float32)
        #svec[ self.itemmap[input_item_id] ] = self.session_count
        svec[ self.itemmap[ items ] ] = 1
                
        predictions = self.predictf( svec, self.itemmap[items[-1]] ) #, self.usermap[input_user_id] )
                
        # Create things in the format
        res_dict = {}
        res_dict['track_id'] =  self.itemmap.index
        res_dict['confidence'] = predictions
        res = pd.DataFrame.from_dict(res_dict)
        res.sort_values( 'confidence', ascending=False, inplace=True )
        res[ np.in1d( res.track_id, items, invert=True ) ]
