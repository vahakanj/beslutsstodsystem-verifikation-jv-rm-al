...
class GRU4Rec:
    ...
    def __init__(self, loss, final_act, hidden_act, layers,
                 ...
                 adapt='adagrad', decay=0.9, grad_cap=0,
                 sigma=0, init_as_normal=False, reset_after_session=True, train_random_order=False, time_sort=True,
                 session_key='playlist_id', item_key='track_id', time_key='pos'):
        self.layers = layers
    ...
    def model(self, X, H, Y=None, drop_p_hidden=0.0, drop_p_embed=0.0, predict=False):
        ...
        if Y is not None:
            ...
            #Wy2 = theano.printing.Print('Y')(self.Wy)
            Sy = self.Wy[Y]
            SBy = self.By[Y]
    ...
    def train(self, train, test=None, retrain=False, sample_store=10000000, callback=None ):
        '''
        Trains the network.
        ...
        train_function = function(inputs=[X, Y], outputs=cost, updates=updates, allow_input_downcast=True)#, profile=True)
        #train_function.profile.summary()
        ...
        for epoch in range(self.n_epochs):
            ...
            if np.isnan(avgc):
                ...
                self.error_during_train = True
                return
            print('Epoch{}\tloss: {:.6f} in {}s'.format(epoch, avgc, ( time.time() - tstart ) ))
    ...
    def predict_next_batch(self, session_ids, input_item_ids, predict_for_item_ids=None, batch=100):
        ...
        if self.predictf is None or self.predict_batch!=batch:
            X = T.ivector()
            Y = T.ivector()
            for i in range(len(self.layers)):
            ...
            if predict_for_item_ids is not None:
                self.predictf = function(inputs=[X, Y], outputs=yhat, updates=updatesH, allow_input_downcast=True)
            else:
                self.predictf = function(inputs=[X], outputs=yhat, updates=updatesH, allow_input_downcast=True)
        ...
        if predict_for_item_ids is not None:
            ...
            preds = np.asarray(self.predictf(in_idxs, iIdxs)).T
            return pd.DataFrame(data=preds, index=predict_for_item_ids)
        else:
            preds = np.asarray(self.predictf(in_idxs)).T
    ...
    def predict( self, name=None, tracks=None, playlist_id=None, artists=None ):
        ...
        for i in range( len( sitems ) - 1 ):
            self.predict_next( playlist_id, sitems[i] )
        
        predictions = self.predict_next( playlist_id, sitems[-1] )
